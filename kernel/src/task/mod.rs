...
pub(crate) struct Vtable {
    ...
    #[expect(
        ...
        reason = "Splitting this up into type aliases just makes it *harder* to understand IMO..."
    )]
...
impl TaskRef {
        ...
        A: Allocator,
    {
        let ptr = Box::into_raw(Box::try_new_in(
            KernelTask::new(future, scheduler, task_id, span),
            alloc,
        )?);
    ...
    {
        let ptr = Box::into_raw(Box::try_new_in(
            WasmTask::new(scheduler, task_id, span),
            alloc,
        )?);
...
impl Drop for TaskRef {
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            {
                let refs = self.state().load(Ordering::Acquire).ref_count();
...
impl<S: Schedule> Schedulable<S> {
    ...
    unsafe fn wake_join_waker(&self) {
        ...
        unsafe {
            let join_waker = self.header.join_waker.get().assume_init_read();
...
// === impl Schedulable Waker construction ===
