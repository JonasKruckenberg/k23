...
use cranelift_codegen::ir::{
    types, GlobalValue, GlobalValueData, InstBuilder, MemFlags, MemoryTypeData, Type,
};
use cranelift_codegen::isa::{TargetFrontendConfig, TargetIsa};
use cranelift_frontend::{FunctionBuilder, FunctionBuilderContext};
use cranelift_wasm::{GlobalVariable, HeapData, TableData, TableSize, TargetEnvironment};
...
impl<'a, 'wasm> cranelift_wasm::ModuleTranslationEnvironment<'wasm> for Environment<'a, 'wasm> {
    fn lookup_type(&self, type_idx: wasmparser::TypeIdx) -> &wasmparser::FunctionType {
        &self.func_translation_env.function_types[type_idx]
            .as_ref()
            .unwrap()
    }
    ...
    fn reserve_globals(&mut self, n: usize) -> Result<(), cranelift_wasm::Error> {
        self.func_translation_env.global_types.resize(n);
        Ok(())
    }
    ...
    fn declare_type(
        ...
        self.func_translation_env.function_types[idx] = Some(ty);
        Ok(())
    }
    ...
    fn declare_memory(
        ...
        self.func_translation_env.memory_types[idx] = Some(memory_type);
        Ok(())
    }
    ...
    fn declare_global(
        ...
        self.func_translation_env.global_types[idx] = Some(global);
        Ok(())
    }
...
struct FunctionEnvironment<'a, 'wasm> {
    ...
    function_types: SecondaryMap<wasmparser::TypeIdx, Option<wasmparser::FunctionType<'wasm>>>,
    global_types: SecondaryMap<wasmparser::GlobalIdx, Option<wasmparser::Global<'wasm>>>,
    memory_types: SecondaryMap<wasmparser::MemIdx, Option<wasmparser::MemoryType>>,
    table_types: SecondaryMap<wasmparser::TableIdx, Option<wasmparser::TableType>>,

    tables: PrimaryMap<cranelift_wasm::Table, TableData>,
...
impl<'a, 'wasm> FunctionEnvironment<'a, 'wasm> {
    pub fn new(target_isa: &'a dyn TargetIsa, memory_max_size: u64) -> Self {
        Self {
            target_isa,
            memory_max_size,
            function_types: SecondaryMap::new(),
            global_types: SecondaryMap::new(),
            memory_types: SecondaryMap::new(),
            table_types: SecondaryMap::new(),
        ...
        for inst in expr.instructions() {
            match inst? {
                ...
                Instruction::GlobalGet { global } => {
                    let global = self.global_types[global].as_ref().unwrap();
                    // ensure!(
...
impl<'a, 'wasm> cranelift_wasm::FuncTranslationEnvironment for FunctionEnvironment<'a, 'wasm> {
    fn lookup_type(&self, type_idx: wasmparser::TypeIdx) -> &wasmparser::FunctionType {
        self.function_types[type_idx].as_ref().unwrap()
    }
    ...
    fn make_global(
        ...
        let global = self.global_types[idx].as_ref().unwrap();
    ...
    fn make_heap(
        ...
        let memory = self.memory_types[idx].as_ref().unwrap();
    ...
    fn tables(&self) -> &PrimaryMap<cranelift_wasm::Table, TableData> {
        &self.tables
    }
    ...
    fn make_table(
        ...
        func: &mut ir::Function,
        idx: wasmparser::TableIdx,
    ) -> Result<cranelift_wasm::Table, cranelift_wasm::Error> {
        todo!();
        // let table = self.table_types[idx].as_ref().unwrap();
        //
        // let (min_size, max_size) = match table.limits {
        //     Limits::Unbounded(min) => (min, wasmparser::MAX_WASM_TABLE_ENTRIES as u32),
        //     Limits::Bounded(min, max) => (
        //         min,
        //         core::cmp::min(max, wasmparser::MAX_WASM_TABLE_ENTRIES as u32),
        //     ),
        // };
        //
        // let base = func.create_global_value(GlobalValueData::IAddImm {
        ...
        // });
        //
        // let bound = if min_size == max_size {
        //     TableSize::Static { bound: min_size }
        // } else {
        //     // TableSize::Dynamic {
        ...
        //     ReferenceType::FuncRef => self.target_config().pointer_bytes() as u32,
        //     ReferenceType::ExternRef => match self.target_config().pointer_type() {
        //         types::I32 => types::R32.bytes(),
        //         types::I64 => types::R64.bytes(),
        //         _ => panic!("unsupported pointer type"),
        //     },
        // };
        //
        // Ok(self.tables.push(TableData {
        //     base,
        //     bound,
        //     element_size,
        //     memory_type: None,
    ...
    fn translate_table_copy(
        ...
        dst_table: cranelift_wasm::Table,
        src_table_index: wasmparser::TableIdx,
        src_table: cranelift_wasm::Table,
        dst: ir::Value,
        src: ir::Value,
        len: ir::Value,
    ) -> Result<(), cranelift_wasm::Error> {
