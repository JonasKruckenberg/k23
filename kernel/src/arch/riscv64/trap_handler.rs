...
declare_thread_local! {
    static TRAP_STACK: [u8; kconfig::TRAP_STACK_SIZE_PAGES * arch::PAGE_SIZE] = const { [0; kconfig::TRAP_STACK_SIZE_PAGES * arch::PAGE_SIZE] };
}
...
pub fn init() {
    let trap_stack_top = unsafe {
        TRAP_STACK
            .as_ptr()
            .byte_add(kconfig::TRAP_STACK_SIZE_PAGES * arch::PAGE_SIZE) as *mut u8
    };
