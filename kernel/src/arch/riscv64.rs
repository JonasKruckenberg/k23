...
use vmm::{AddressRangeExt, BumpAllocator, EntryFlags, Mapper, Mode, VirtualAddress};
...
pub unsafe extern "C" fn kstart(args_ptr: *const KernelArgs) -> ! {
    ...
    let info = BootInfo::from_dtb(args.fdt);

    let mut alloc = BumpAllocator::new(info.memories[0].clone(), args.alloc_offset);
    let mut mapper: Mapper<kconfig::MEMORY_MODE> = Mapper::from_active(0, &mut alloc);

    let serial_phys = info.serial.regs.clone().align(kconfig::PAGE_SIZE);
    let serial_virt = kconfig::MEMORY_MODE::phys_to_virt(serial_phys.start)
        ..kconfig::MEMORY_MODE::phys_to_virt(serial_phys.end);
    ...
    crate::logger::init(serial_virt.start, info.serial.clock_frequency);
    ...
    //
    //         dtb_parser::DevTree::from_raw(args.fdt)
    //             .unwrap()
    //             .visit(&mut v)
    //             .unwrap();
    //     }
    ...
    // });
    //
    // crate::kmain(args.boot_hart, info);
}
