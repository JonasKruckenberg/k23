...
use crate::frame_alloc::frame::{Frame, FrameState, Unique};
...
pub struct ArenaSelection {
    arena: Range<PhysicalAddress>,
    bookkeeping: Range<PhysicalAddress>,
    wasted_bytes: usize,
}
...
impl fmt::Debug for Arena {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Arena")
            ...
            .field(
                "frames",
                &format_args!("&[MaybeUninit<Frame>; {}]", self.frames.len()),
            )
...
impl Arena {
    pub fn from_selection(selection: ArenaSelection, phys_off: VirtualAddress) -> Self {
        ...
        while remaining_bytes > 0 {
...

            {
                ...
                let frame = raw_frames[idx].write(Frame {
                    links: Default::default(),
                    phys: addr,
                    state: FrameState::Free,
                });
                free_lists[order].push_back(Unique::from(frame).into_pin());
            }
    ...
    pub fn allocate_contiguous(
        ...
        self.allocate_inner(size, list)
    }

    fn allocate_inner(
        &mut self,
        size: usize,
        out: &mut linked_list::List<Frame>,
    ) -> crate::Result<()> {
        ...
        for order in (min_order + 1..page_order + 1).rev() {
            // Insert the "upper half" of the block into the free list.
            let buddy_addr = frame.phys.checked_add(PAGE_SIZE << (order - 1)).unwrap();
            let buddy = self.find_specific(buddy_addr).unwrap().write(Frame {
                links: Default::default(),
                phys: buddy_addr,
                state: FrameState::Free,
            });
            let buddy = Unique::from(buddy).into_pin();
            self.free_lists[order - 1].push_back(buddy);
        }

        let frames_uninit = unsafe {
            slice::from_raw_parts_mut(
                Pin::into_inner_unchecked(frame).as_ptr().cast(),
                size_frames,
            )
        };

        let mut phys = frame.phys;
        let frames = MaybeUninit::fill_with(frames_uninit, || {
            let frame = Frame {
                links: Default::default(),
                phys,
        ...
        out.extend(
            frames
                .iter_mut()
                .map(|frame| unsafe { Pin::new_unchecked(Unique::from(frame)) }),
        );
    ...
    fn find_specific(&mut self, addr: PhysicalAddress) -> Option<&mut MaybeUninit<Frame>> {
        debug_assert!(addr.is_aligned_to(PAGE_SIZE));
        let offset = addr.checked_sub_addr(self.range.start).unwrap();
        let idx = offset / PAGE_SIZE;
