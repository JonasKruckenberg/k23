...
use crate::wasm_rt::runtime::builtins::VMBuiltinFunctionsArray;
use crate::wasm_rt::runtime::memory::Memory;
use crate::wasm_rt::runtime::table::Table;
use crate::wasm_rt::runtime::vmcontext::{
    VMArrayCallFunction, VMGlobalDefinition, VMWasmCallFunction,
};
use crate::wasm_rt::runtime::{
...
use crate::wasm_rt::translate::{TableInitialValue, TableSegmentElements};
use crate::wasm_rt::{Extern, Module, Store};
use alloc::vec;
use alloc::vec::Vec;
...
impl Instance {
    ...
    pub unsafe fn new_unchecked(
        ...
        unsafe {
            arch::with_user_memory_access(|| -> crate::wasm_rt::Result<()> {
                initialize_vmctx(
                    const_eval,
                    &mut vmctx,
                    &mut tables,
