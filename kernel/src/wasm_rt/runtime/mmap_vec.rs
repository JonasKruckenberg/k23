...
impl<T> MmapVec<T> {
    ...
    pub fn new_zeroed(aspace: &mut AddressSpace, capacity: usize) -> crate::wasm_rt::Result<Self> {
        Ok(Self {
    ...
    pub fn from_slice(aspace: &mut AddressSpace, slice: &[T]) -> crate::wasm_rt::Result<Self>
    where
        T: Clone,
    {
        if slice.is_empty() {
