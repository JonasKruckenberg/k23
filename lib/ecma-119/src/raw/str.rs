...
impl<const N: usize> DStr<N> {
    /// Creates a `DStr` from raw bytes, validating d-character constraints.
    ///
    /// # Errors
    ///
    /// Returns an error if any byte is not a valid d-character.
    ...
    /// Returns the contents as a UTF-8 string.
    ///
    /// # Errors
    ///
...
impl<const N: usize> AStr<N> {
    /// Creates an `AStr` from raw bytes, validating a-character constraints.
    ///
    /// # Errors
    ///
    /// Returns an error if any byte is not a valid a-character.
    pub fn try_from_bytes(bytes: [u8; N]) -> anyhow::Result<Self> {
        let me = Self(bytes);
        me.validate()?;
        Ok(me)
...
impl<const N: usize> Validate for FileId<N> {
    fn validate(&self) -> anyhow::Result<()> {
        ...
        for (i, &b) in bytes.iter().enumerate() {
            anyhow::ensure!(
                is_file_id_char(b),
                "FileId: invalid character {b:#04x} ({}) at position {i}",
                char::from(b),
            );
        }
        Ok(())
...
impl<const N: usize> FileId<N> {
    ...
    /// # Errors
    ///
    /// Returns an error if any byte is not a valid file identifier character.
    pub fn try_from_bytes(bytes: [u8; N]) -> anyhow::Result<Self> {
        let me = Self(bytes);
        me.validate()?;
        Ok(me)
