...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn volume_descriptor_set(&mut self) -> anyhow::Result<VolumeDescriptorSet<'a>> {
        ...
        loop {
            anyhow::ensure!(
                self.pos.is_multiple_of(SECTOR_SIZE),
                "volume descriptor must be at a sector boundary (pos={}, remainder={})",
                self.pos,
                self.pos % SECTOR_SIZE,
            );
            // The descriptor header carries the "is this an ISO 9660 image?"
            // magic (standard_id == "CD001"). Always validate it, even in
            // lenient mode.
        ...
        Ok(VolumeDescriptorSet {
            ...
            _enhanced: enhanced,
            _volume_partition: volume_partition,
        })
    }
}
