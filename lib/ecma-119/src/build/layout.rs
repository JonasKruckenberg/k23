...
use super::directory::{Directory, File};
use crate::build::boot::BootEntry;
use crate::build::directory::Entry;
use crate::build::{BootConfig, FileSource};
use crate::eltorito::{InitialEntry, SectionEntry, SectionHeaderEntry, ValidationEntry};
use crate::raw;
use crate::raw::{
    BothEndianU16, BothEndianU32, DirDateTime, DirectoryRecordHeader, FileFlags,
    PathTableRecordHeader, RootDirectoryRecord, SECTOR_SIZE, VIRTUAL_SECTOR_SIZE,
...
impl<'a> Layout<'a> {
    pub(super) fn flatten(root: Directory<'a>) -> Self {
        let mut dirs = Vec::new();
        let mut files: Vec<FileNode<'a>> = Vec::new();
        let mut path_table_size = 0;

        // Queue holds (parent_index_in_dirs, directory_to_process).
        let mut queue: VecDeque<(u16, Directory<'a>)> = VecDeque::new();
        ...
        let dir = DirNode::new(Cow::Borrowed("\0"), 0, root.entries.len());
        path_table_size += dir.required_path_table_record_size();
        dirs.push(dir);
        queue.push_back((0, root));

        while let Some((parent_idx, dir)) = queue.pop_front() {
            for (name, entry) in dir.entries {
                match entry {
                    Entry::Subdir(subdir) => {
                        let child_idx = u16::try_from(dirs.len()).expect("too many directories");

                        let entries_hint = subdir.entries.len();
                        let child = DirNode::new(name.clone(), parent_idx, entries_hint);

                        // update the required path table size to hold this directory
                        path_table_size += child.required_path_table_record_size();
                        // update our parent so it knows where this directory is
                        dirs[parent_idx as usize].attach_child_directory(name, child_idx);
                    ...
                    Entry::File(File { source }) => {
                        let file_idx = u32::try_from(files.len()).expect("too many files in image");
                        files.push(FileNode::new(name.clone(), source));
                        dirs[parent_idx as usize].attach_file(name, file_idx);
                    }
                }
            }
        }
        ...
        for d in &dirs {
            debug_assert!(
                d.sorted_entry_ids.windows(2).all(|w| w[0].0 <= w[1].0),
                "DirNode::sorted_entry_ids must be sorted by identifier (dir={:?})",
                d.identifier,
            );
        }
    ...
    pub(super) fn assign_lbas(
        &mut self,
        boot_config: Option<&mut BootConfig<'_>>,
    ) -> io::Result<()> {
        // On-disk layout:
        //
        ...
        if let Some(ref boot_config) = boot_config {
        ...
        lba += sectors_for(self.path_table_size);
        ...
        // Now that all file LBAs are assigned, resolve each boot entry's path
        ...
        if let Some(boot_config) = boot_config {
            let resolve = |entry: &mut BootEntry<'_>| -> io::Result<()> {
                let boot_image = self.find_file(entry.boot_image_path).ok_or_else(|| {
                    io::Error::new(
                        ...
                        format!(
                            "boot image path {:?} not found in directory tree",
                ...
                if entry.load_size.is_none() {
                    ...
                    entry.boot_image_sector_count =
                        u16::try_from(virtual_sectors).map_err(|_| {
                            io::Error::new(
                                io::ErrorKind::InvalidInput,
                                format!(
                                    "boot image {:?} requires {} virtual sectors (> u16::MAX); set BootEntry::load_size to override",
                                    entry.boot_image_path, virtual_sectors,
                                ),
                            )
                        })?;
                }
            ...
            resolve(&mut boot_config.default_entry)?;
            for section in &mut boot_config.sections {
    ...
    pub(crate) fn serialize(
        ...
        //   sectors 0..16   system area (zeroed)
        //   16              PVD
        //   17              Boot Record VD  (only if boot)
        //   17 or 18        VD Set Terminator
        //   ...             boot catalog    (only if boot)
        //   ...             Path Table L, then Path Table M
        //   ...             every directory extent, in BFS order
        //   ...             every file's data, in BFS order
        ...
        // sector 17: Boot Record VD (El Torito), then VD Set Terminator at 18.
        // Without boot the terminator sits at 17.
        if let Some(boot_config) = boot_config {
            let boot_catalog_lba = self.boot_catalog_lba.expect("boot_catalog_lba must be set");
            let mut boot_record = raw::BootRecord::new_zeroed();
            boot_record.boot_system_id.0[..23].copy_from_slice(b"EL TORITO SPECIFICATION");
            boot_record.boot_catalog = U32::new(boot_catalog_lba);
            write_descriptor(&mut w, 0, &boot_record)?;
        ...
        for i in 0..self.dirs.len() {
            serialize_dir_extent(&mut w, &self.dirs[i], &self.dirs, &self.files)?;
        }
    ...
    fn find_file(&self, path: &str) -> Option<&FileNode<'a>> {
        ...
        loop {
            ...
            if parts.peek().is_none() {
                // last component — must be a file in the current directory
                let pos = self.dirs[dir_idx]
                    .sorted_entry_ids
                    .binary_search_by_key(&component, |(id, _)| id.as_ref())
                    .ok()?;

                let (_, DirNodeEntry::File(file_idx)) = &self.dirs[dir_idx].sorted_entry_ids[pos]
                else {
            ...
            let entries = &self.dirs[dir_idx].sorted_entry_ids;
            let pos = entries
                .binary_search_by_key(&component, |(id, _)| id.as_ref())
                .ok()?;
            let child_idx = match entries[pos].1 {
                DirNodeEntry::ChildDirectory(idx) => idx as usize,
                DirNodeEntry::File(_) => return None, // path component is a file, not a dir
            };
            dir_idx = child_idx;
        }
    }
...
/// Intermediate representation of a directory for layout purposes.
/// We create this from the hierarchical tree the caller gave us and pass this
/// through the layout and serialization stages.
#[derive(Debug)]
pub(super) struct DirNode<'a> {
    ...
    sorted_entry_ids: Vec<(Cow<'a, str>, DirNodeEntry)>,
}
...
/// Intermediate representation of a file for layout purposes.
/// We create this from the hierarchical tree the caller gave us and pass this
/// through the layout and serialization stages.
#[derive(Debug)]
...
impl<'a> FileNode<'a> {
    fn new(identifier: Cow<'a, str>, source: FileSource<'a>) -> Self {
        Self {
            lba: 0,
            len: source.len(),
            identifier,
...
impl<'a> DirNode<'a> {
    fn new(identifier: Cow<'a, str>, parent_directory: u16, children_size_hint: usize) -> Self {
        Self {
            extent_lba: 0,
            extent_len: 0,
            identifier,
    ...
    fn attach_child_directory(&mut self, name: Cow<'a, str>, child_idx: u16) {
        self.sorted_entry_ids
            .push((name, DirNodeEntry::ChildDirectory(child_idx)));
    }

    fn attach_file(&mut self, name: Cow<'a, str>, file_idx: u32) {
        self.sorted_entry_ids
    ...
    fn files<'l>(&self, all_files: &'l [FileNode<'a>]) -> impl Iterator<Item = &'l FileNode<'a>> {
        self.sorted_entry_ids.iter().filter_map(|(_, e)| {
            if let DirNodeEntry::File(idx) = e {
                Some(&all_files[*idx as usize])
            } else {
                None
            }
        })
    ...
    fn directories<'l>(
        &self,
        all_dirs: &'l [DirNode<'a>],
    ) -> impl Iterator<Item = &'l DirNode<'a>> {
        self.sorted_entry_ids.iter().filter_map(|(_, e)| {
            if let DirNodeEntry::ChildDirectory(idx) = e {
                Some(&all_dirs[*idx as usize])
            } else {
                None
            }
        })
...
fn write_descriptor_header(w: &mut impl io::Write, ty: u8) -> io::Result<()> {
    VolumeDescriptorHeader {
...
/// Writes a `VolumeDescriptorHeader` followed by the descriptor body.
/// Together they must fill exactly one sector; the const assertions on each
/// descriptor type in `raw.rs` guarantee this.
fn write_descriptor<T: IntoBytes + zerocopy::Immutable>(
    w: &mut impl io::Write,
    ty: u8,
    body: &T,
) -> io::Result<()> {
    write_descriptor_header(w, ty)?;
...
#[inline]
fn dir_record_size(id_len: u8) -> u8 {
...
fn build_dir_record_header(
    ...
    let len = dir_record_size(id_len);
...
fn write_boot_catalog(w: &mut impl io::Write, boot_config: &BootConfig) -> io::Result<()> {
    ...
    let validation = {
        ...
        e.header_id = 1;
        e.platform_id = u8::from(boot_config.validation_platform);
        e.key = [0x55, 0xAA];
        // Compute checksum with all other fields set; negate so the total sum is zero.
        let bytes = e.as_bytes();
        let sum = bytes
            .chunks_exact(2)
            .map(|w| u16::from_le_bytes([w[0], w[1]]))
            .fold(0u16, u16::wrapping_add);
        e.checksum = U16::new(0u16.wrapping_sub(sum));
        e
    };
    validation.write_to_io(&mut *w)?;

    // InitialEntry — the default boot entry (required).
    write_initial_entry(w, &boot_config.default_entry)?;

    // Section headers + entries.
    let section_count = boot_config.sections.len();
    for (i, section) in boot_config.sections.iter().enumerate() {
        let is_last = i == section_count - 1;
        let mut header = SectionHeaderEntry::new_zeroed();
        ...
        for entry in &section.entries {
            let mut se = SectionEntry::new_zeroed();
            se.bootable = if entry.bootable { 0x88 } else { 0x00 };
            se.boot_media_ty = u8::from(entry.emulation);
            se.sector_count = U16::new(entry.resolved_sector_count());
            se.load_rba = U32::new(entry.boot_image_lba);
            se.write_to_io(&mut *w)?;
        }
    }
...
fn write_initial_entry(w: &mut impl io::Write, entry: &BootEntry<'_>) -> io::Result<()> {
    let mut ie = InitialEntry::new_zeroed();
    ie.boot_indicator = if entry.bootable { 0x88 } else { 0x00 };
    ie.boot_media_ty = u8::from(entry.emulation);
    ie.sector_count = U16::new(entry.resolved_sector_count());
    ie.load_rba = U32::new(entry.boot_image_lba);
    ie.write_to_io(w)
}
