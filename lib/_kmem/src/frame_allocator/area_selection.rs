...
/// Returned `ArenaSelections` indicating the provided regions are unsuitable for allocation
/// either because they are too small to even hold the necessary bookkeeping data.
#[derive(Debug)]
...
) -> impl FallibleIterator<Item = AreaSelection, Error = SelectionError> {
    ArenaSelections {
        allocatable_regions,
        wasted_bytes: 0,
...
struct ArenaSelections<A: RawAddressSpace> {
    allocatable_regions: SmallVec<[Range<PhysicalAddress>; 4]>,
    wasted_bytes: usize,
...
impl<A: RawAddressSpace> FallibleIterator for ArenaSelections<A> {
    ...
    fn next(&mut self) -> Result<Option<Self::Item>, Self::Error> {
        ...
        Ok(Some(AreaSelection {
            area: aligned,
            bookkeeping: bookkeeping_start
                ..bookkeeping_start
                    .checked_add(bookkeeping_size_frames * A::PAGE_SIZE)
                    .unwrap(),
...
impl core::fmt::Display for SelectionError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        write!(f, "range {:#x?} is too small for allocation.", self.range)
    }
}
