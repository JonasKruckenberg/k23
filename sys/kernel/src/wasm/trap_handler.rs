...
where
    F: for<'a> FnOnce(&'a unwind::Registers),
{
    extern "C" fn trampoline<F>(saved: &mut unwind::Registers, env: *mut ())
    where
        F: for<'a> FnOnce(&'a unwind::Registers),
    {
    ...
    unwind::save_context(trampoline::<F>, env);
}
...
pub struct Activation {
    ...
    /// Register context to non-locally jump back to. Captured on wasm-entry
    /// by [`call_with_saved_registers`] and consumed either by the trap handler
    /// or by the explicit [`raise_preexisting_trap`].
    regs: NonNull<unwind::Registers>,
...
impl Activation {
    pub fn new(store: &mut StoreOpaque, regs: &unwind::Registers) -> Self {
        Self {
    ...
    unsafe fn unwind(&self) -> ! {
        // Safety: ensured by caller
        unsafe { unwind::restore_context(self.regs.as_ref()) }
    }
    ...
    fn trace_through_wasm(
        ...
        while fp != trampoline_fp {
            ...
            #[expect(
                ...
                reason = "fp points to an 8-byte-aligned stack slot holding the saved FP"
            )]
            #[expect(clippy::undocumented_unsafe_blocks, reason = "")]
