...
use tracing::field;
use tracing_core::{
    Callsite, Event, Kind, Level, Metadata, Subscriber as Collect, identify_callsite,
};

impl log::Log for super::Subscriber {
    fn enabled(&self, metadata: &log::Metadata) -> bool {
        if let Some(level_filter) = Collect::max_level_hint(self) {
    ...
    fn log(&self, record: &Record) {
        ...
        if !Collect::enabled(self, &filter_meta) {
            return;
        }
