...
pub fn init(filter: Filter) {
    ...
    // tracing-core 0.1.x has no `Dispatch::from_static`, so we wrap the static
    // subscriber in a ZST that delegates to it. `Dispatch::new` allocates via
    // `Arc::new`, which is why this is deferred until after the allocator is up.
    dispatch::set_global_default(Dispatch::new(StaticSub)).unwrap();
}

struct StaticSub;

impl Collect for StaticSub {
    fn register_callsite(&self, meta: &'static Metadata<'static>) -> Interest {
        SUBSCRIBER.get().unwrap().register_callsite(meta)
    }

    fn enabled(&self, meta: &Metadata<'_>) -> bool {
        SUBSCRIBER.get().unwrap().enabled(meta)
    }

    fn max_level_hint(&self) -> Option<LevelFilter> {
        SUBSCRIBER.get().unwrap().max_level_hint()
    }

    fn new_span(&self, attrs: &Attributes<'_>) -> Id {
        SUBSCRIBER.get().unwrap().new_span(attrs)
    }

    fn record(&self, span: &Id, values: &Record<'_>) {
        SUBSCRIBER.get().unwrap().record(span, values);
    }

    fn record_follows_from(&self, span: &Id, follows: &Id) {
        SUBSCRIBER.get().unwrap().record_follows_from(span, follows);
    }

    fn event(&self, event: &Event<'_>) {
        SUBSCRIBER.get().unwrap().event(event);
    }

    fn enter(&self, id: &Id) {
        SUBSCRIBER.get().unwrap().enter(id);
    }

    fn exit(&self, id: &Id) {
        SUBSCRIBER.get().unwrap().exit(id);
    }

    fn clone_span(&self, id: &Id) -> Id {
        SUBSCRIBER.get().unwrap().clone_span(id)
    }

    fn try_close(&self, id: Id) -> bool {
        SUBSCRIBER.get().unwrap().try_close(id)
    }

    fn current_span(&self) -> Current {
        SUBSCRIBER.get().unwrap().current_span()
    }
}
