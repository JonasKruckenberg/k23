...
| `lib/` | First-party reusable crates (`riscv`, `trap`, `uart-16550`, `cpu-local`, `mem-core`, `wavltree`, `unwind`, `spin`, `fdt`, …). New subsystems start here unless irreducibly kernel-internal. |
| `sys/panic-unwind/`, `sys/backtrace/` | Kernel-only policy: the `#[panic_handler]`/`catch_unwind` runtime and the symbolizing backtrace subsystem. Not reusable libraries — they build on `lib/unwind`. |
