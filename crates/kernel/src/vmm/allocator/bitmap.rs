use crate::vmm::allocator::bump::BumpAllocator;
use crate::vmm::allocator::{FrameAllocator, FrameUsage};
use crate::vmm::{Mode, PhysicalAddress, VirtualAddress};
use crate::Error;
...
impl<M: Mode> BitMapAllocator<M> {
    ...
    fn entries_mut(&self) -> &mut [TableEntry<M>] {
        unsafe {
            core::slice::from_raw_parts_mut(
                self.table_virt.0 as *mut TableEntry<M>,
                Self::NUM_ENTRIES,
            )
        }
    }
    ...
    fn entries(&self) -> &[TableEntry<M>] {
        unsafe {
            core::slice::from_raw_parts(
                self.table_virt.0 as *const TableEntry<M>,
                Self::NUM_ENTRIES,
            )
        }
    }
    ...
    pub fn new(mut bump_allocator: BumpAllocator<M>) -> crate::Result<Self> {
        ...
        let table_virt = unsafe { M::phys_to_virt(table_phys) };
