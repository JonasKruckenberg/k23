...
use cranelift_codegen::ir::{
    types, GlobalValue, GlobalValueData, InstBuilder, MemoryTypeData, TableData,
};
use cranelift_codegen::isa::{TargetFrontendConfig, TargetIsa};
use cranelift_codegen::{ir, CompiledCode};
use cranelift_frontend::{FunctionBuilder, FunctionBuilderContext};
use cranelift_wasm::{GlobalVariable, HeapData, TargetEnvironment};
use kmem::{AddressRange, Arch, EntryFlags, Mapper, VirtualAddress};
use wasmparser::{Limits, Mutability, ReferenceType, ValueType};

const MAX_MEMORY_SIZE: u64 = 0x0000003fffffffff;

const USERSPACE_START: VirtualAddress = unsafe { VirtualAddress::new(0).add(2 * MIB) };

pub fn compile_wasm(bytes: &[u8]) -> crate::Result<()> {
    let compiler = cranelift_wasm::Compiler::new_for_host()?;

    let mut env = Environment::new(compiler.target_isa(), MAX_MEMORY_SIZE);

    let module = wasmparser::parse_module(bytes)?;

    cranelift_wasm::translate_module(module, &mut env)?;
    ...
    while let Some((_idx, Some(func))) = iter.next() {
        ...
        let compiled_func = compiler.compile_function(func)?;

        assert!(compiled_func.buffer.relocs().is_empty());

        compiled.push_func(compiled_func);
    }

    log::debug!("successfully compiled module {compiled:?}");

    // map the compiled module into memory

    log::debug!("here");
    let mut frame_alloc = arch::paging::FRAME_ALLOC.wait().lock();
    let mut mapper = Mapper::from_active(0, frame_alloc.deref_mut(), VMM::PHYS_OFFSET);

    let phys_range = {
        let num_frames = if compiled.text_segment_size / VMM::PAGE_SIZE > 0 {
            compiled.text_segment_size / VMM::PAGE_SIZE
        } else {
            1
        };
        let base = mapper.allocator_mut().allocate_frames(num_frames)?;
        (base..base.add(compiled.text_segment_size)).align(VMM::PAGE_SIZE)
    };
    let virt_range = USERSPACE_START..USERSPACE_START.add(phys_range.size_in_bytes());

    log::debug!(
        "mapping module into memory... {:?} => {:?}",
        virt_range,
        phys_range
    );
    // let flush = mapper.map_range(
    //     virt_range.clone(),
    //     phys_range,
    //     EntryFlags::READ | EntryFlags::WRITE | EntryFlags::EXECUTE,
    // )?;
    // flush.flush()?;
    // log::debug!("mapping success!");

    // log::debug!("copying into memory...");
    // let slice = unsafe {
    //     core::slice::from_raw_parts_mut(
    //         virt_range.start.as_raw() as *mut u8,
    //         compiled.text_segment_size,
    //     )
    // };
    // slice.copy_from_slice(&compiled.funcs[0].code_buffer());
    // log::debug!("copying complete");
    //
    // log::debug!("calling func at {:?}!", virt_range.start.as_raw());
    // let ptr = virt_range.start.as_raw() as *const u8;
    // unsafe {
    //     // let func = mem::transmute::<_, unsafe extern "C" fn()>(ptr);
    //     // func()
    // }
    // log::debug!("success calling func!");
...
struct CompiledModule {
    text_segment_size: usize,
    funcs: Vec<CompiledCode>,
}

impl CompiledModule {
    pub fn push_func(&mut self, func: CompiledCode) {
        self.text_segment_size += func.code_buffer().len();
