...
def _kcfg_rust_impl(ctx):
    out = ctx.actions.declare_output(ctx.attrs.out)
    ctx.actions.write(out, ctx.attrs.content)
    return [DefaultInfo(default_output = out)]

_kcfg_rust = rule(
    impl = _kcfg_rust_impl,
    attrs = {
        "content": attrs.string(),
        "out":     attrs.string(default = "lib.rs"),
    },
)

def kcfg_rust(name, consts, out = "kcfg.generated.rs", **kwargs):
    """Declare a target that writes a Rust module of `pub const`s.

    consts – list of (rust_name, kcfg.declare() return value) pairs. Values are
             read at load time, so `--config` works but a per-platform `select()`
             on them does not.

    Only int options are supported: they are the ones that need to reach Rust as
    a compile-time constant. Everything else stays a `select()` on a
    `config_setting`.
    """
    # Line comments, not inner doc comments: this file is `include!`d into a
    # module, where an inner attribute would not be in a legal position.
    lines = ["// @generated by //build:kcfg.bzl — do not edit."]

    for rust_name, decl in consts:
        if decl.type_name != "int":
            fail("kcfg_rust: {}.{} is `{}`, expected `int`".format(decl.section, decl.key, decl.type_name))

        lines += [
            "",
            "/// {}".format(decl.doc),
            "///",
            "/// Configure with `--config {}.{}=VALUE`.".format(decl.section, decl.key),
            "pub const {}: usize = {};".format(rust_name, decl.read()),
        ]

    _kcfg_rust(name = name, content = "\n".join(lines) + "\n", out = out, **kwargs)
