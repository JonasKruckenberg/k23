use crate::strings::{get_str, with_cstr, with_cstrs, with_opt_cstr};
use crate::{
    debug_descr, IsValid, SBCommandInterpreter, SBError, SBFile, SBFrame, SBIterator, SBPlatform,
    SBStringList, SBStructuredData, SBTarget,
};
...
impl SBDebugger {
    ...
    pub fn num_platforms(&self) -> u32 {
        cpp!(unsafe [self as "SBDebugger*"] -> u32 as "uint32_t" {
            return self->GetNumPlatforms();
        })
    }
    pub fn platform_at_index(&self, idx: u32) -> SBPlatform {
        cpp!(unsafe [self as "SBDebugger*", idx as "uint32_t"] -> SBPlatform as "SBPlatform" {
            return self->GetPlatformAtIndex(idx);
        })
    }
    pub fn platforms(&self) -> impl Iterator<Item = SBPlatform> + '_ {
        SBIterator::new(self.num_platforms(), |idx| {
            return self.platform_at_index(idx);
        })
    }
    pub fn num_available_platforms(&self) -> u32 {
        cpp!(unsafe [self as "SBDebugger*"] -> u32 as "uint32_t" {
            return self->GetNumAvailablePlatforms();
        })
    }
    pub fn available_platform_info_at_index(&self, idx: u32) -> SBStructuredData {
        cpp!(unsafe [self as "SBDebugger*", idx as "uint32_t"] -> SBStructuredData as "SBStructuredData" {
            return self->GetAvailablePlatformInfoAtIndex(idx);
        })
    }
    pub fn available_platforms(&self) -> impl Iterator<Item = SBStructuredData> + '_ {
        SBIterator::new(self.num_available_platforms(), |idx| {
            return self.available_platform_info_at_index(idx);
        })
    }
    ...
    pub fn instance_name(&self) -> &str {
        let ptr = cpp!(unsafe [self as "SBDebugger*"] ->  *const ffi::c_char as "const char*" {
    ...
    pub fn set_variable_for(
        debugger_instance_name: &str,
        var_name: &str,
        value: &str,
    ) -> Result<(), SBError> {
        with_cstr(debugger_instance_name, |debugger_instance_name| {
