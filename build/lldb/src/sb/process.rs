...
impl SBProcess {
    ...
    pub fn threads(&self) -> impl Iterator<Item = SBThread> + '_ {
    ...
    pub fn memory_regions(&self) -> SBMemoryRegionInfoList {
        cpp!(unsafe [self as "SBProcess*"] -> SBMemoryRegionInfoList as "SBMemoryRegionInfoList" {
            return self->GetMemoryRegions();
        })
    }
    ...
    pub fn remote_attach_to_process_with_id(&self, pid: u64) -> Result<bool, SBError> {
        let mut error = SBError::new();
        let ok = cpp!(unsafe [self as "SBProcess*", pid as "lldb::pid_t", mut error as "SBError"] -> bool as "bool" {
            return self->RemoteAttachToProcessWithID(pid, error);
        });
    ...
    // RemoteLaunch (char const **argv, char const **envp, const char *stdin_path, const char *stdout_path, const char *stderr_path, const char *working_directory, uint32_t launch_flags, bool stop_at_entry, lldb::SBError &error)
    pub fn remote_launch(&self, stop_at_entry: bool) -> Result<bool, SBError> {
        let mut error = SBError::new();

        let ok = cpp!(unsafe [self as "SBProcess*", stop_at_entry as "bool", mut error as "SBError"] -> bool as "bool" {
            return self->RemoteLaunch(0, 0, 0, 0, 0, 0, 0, stop_at_entry, error);
        });
...
impl ProcessState {
    ...
    pub fn is_running(&self) -> bool {
        matches!(
            self,
            Self::Attaching | Self::Launching | Self::Running | Self::Stepping
        )
    }
    ...
    pub fn is_stopped(&self) -> bool {
        matches!(
            self,
            Self::Stopped | Self::Crashed | Self::Suspended | Self::Unloaded | Self::Exited
        )
    }
}
