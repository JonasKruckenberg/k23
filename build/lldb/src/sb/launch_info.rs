...
use crate::{ProcessID, SBFileSpec, SBIterator, SBListener};
...
impl SBLaunchInfo {
    ...
    pub fn set_arguments<'a>(&mut self, args: impl IntoIterator<Item = &'a str>, append: bool) {
        ...
        let mut ptrs: Vec<*const ffi::c_char> = cstrs.iter().map(|cs| cs.as_ptr()).collect();
        ptrs.push(ptr::null());
    ...
    pub fn argument_at_index(&self, index: u32) -> &str {
        let ptr = cpp!(unsafe [self as "SBLaunchInfo*", index as "uint32_t"] -> *const ffi::c_char as "const char*" {
            return self->GetArgumentAtIndex(index);
        });
        unsafe { get_str(ptr) }
    }
    pub fn arguments(&self) -> impl Iterator<Item = &str> {
        SBIterator::new(self.num_arguments(), move |index| {
            self.argument_at_index(index)
        })
    }
    ...
    pub fn working_directory(&self) -> Option<&Path> {
        let ptr = cpp!(unsafe [self as "SBLaunchInfo*"] -> *const ffi::c_char as "const char*" {
