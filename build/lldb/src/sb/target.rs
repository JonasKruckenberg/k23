...
use crate::{
    debug_descr, Address, BasicType, BreakpointID, ByteOrder, IsValid, ProcessID, SBAddress,
    SBAttachInfo, SBBreakpoint, SBBroadcaster, SBData, SBDebugger, SBError, SBFileSpec,
    SBInstructionList, SBIterator, SBLaunchInfo, SBListener, SBModule, SBModuleSpec, SBPlatform,
    SBProcess, SBSymbolContext, SBSymbolContextList, SBType, SBValue, SBValueList, SBWatchpoint,
    SymbolType, WatchpointID,
};
...
impl SBTarget {
    ...
    pub fn launch_redirected(
        &self,
        stdout_path: &str,
        stderr_path: &str,
        args: impl IntoIterator<Item = String>,
    ) -> Result<(SBProcess, SBListener), SBError> {
        let mut error = SBError::new();
        with_cstr(stdout_path, |stdout_path| {
            with_cstr(stderr_path, |stderr_path| {
                let listener = SBListener::new();
                let l = &listener;
                let cstrs: Vec<CString> =
                    args.into_iter().map(|a| CString::new(a).unwrap()).collect();
                let mut ptrs: Vec<*const ffi::c_char> =
                    cstrs.iter().map(|cs| cs.as_ptr()).collect();
                ptrs.push(ptr::null());
                let argv = ptrs.as_ptr();

                let process = cpp!(unsafe [self as "SBTarget*", l as "SBListener*", argv as "const char**", stdout_path as "const char*", stderr_path as "const char*", mut error as "SBError"] -> SBProcess as "SBProcess" {
                    return self->Launch(
                        *l,
                        argv,
                        nullptr, // envp
    ...
    pub fn connect_remote(&self, url: &str, plugin_name: &str) -> Result<SBProcess, SBError> {
        let error = SBError::new();
        let listener = SBListener::new();

        with_cstr(url, |url| {
            with_cstr(plugin_name, |plugin_name| {
                let process = {
                    let ref_error = &error;
                    let ref_listener = &listener;

                    cpp!(unsafe [self as "SBTarget*", ref_listener as "SBListener*", url as "const char*", plugin_name as "const char*", ref_error as "SBError*"] -> SBProcess as "SBProcess" {
                        return self->ConnectRemote(*ref_listener, url, plugin_name, *ref_error);
                    })
                };
                if error.is_success() {
                    Ok(process)
    ...
    pub fn find_global_variables(&self, name: &str) -> SBValueList {
        with_cstr(name, |name| {
            let max = u32::MAX;
            cpp!(unsafe [self as "SBTarget*", name as "const char*", max as "uint32_t"]
                        -> SBValueList as "SBValueList" {
                return self->FindGlobalVariables(name, max);
            })
        })
    }
    ...
    pub fn broadcaster_class_name() -> &'static str {
        let ptr = cpp!(unsafe [] -> *const ffi::c_char as "const char*" {
    ...
    pub fn triple(&self) -> &str {
        let ptr = cpp!(unsafe [self as "SBTarget*"] -> *const ffi::c_char as "const char*" {
