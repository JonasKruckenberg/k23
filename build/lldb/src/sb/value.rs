...
use crate::{
    debug_descr, Address, IsValid, SBAddress, SBDataOwned, SBError, SBFrame, SBIterator, SBProcess,
    SBStream, SBTarget, SBThread, SBType, UserID,
};
...
impl SBValue {
    ...
    pub fn name(&self) -> Option<&str> {
        let ptr = cpp!(unsafe [self as "SBValue*"] -> *const ffi::c_char as "const char*" {
            return self->GetName();
        });
        if ptr.is_null() {
            None
    ...
    pub fn children(&self) -> impl Iterator<Item = SBValue> + '_ {
...
impl Format {
    ...
    pub const COMPLEX_FLOAT: Format = Format::Complex;
}
