use crate::strings::{get_cstring, get_str, with_cstr};
use crate::{
    debug_descr, Address, IsValid, SBAddress, SBLineEntry, SBThread, SBValue, SBValueList,
    ValueType,
};
...
impl SBFrame {
    pub fn frame_id(&self) -> i32 {
        cpp!(unsafe [self as "SBFrame*"] -> i32 as "const uint32_t" {
            return self->GetFrameID();
        })
    }

    pub fn function_name(&self) -> Option<&str> {
        let ptr = cpp!(unsafe [self as "SBFrame*"] -> *const ffi::c_char as "const char*" {
