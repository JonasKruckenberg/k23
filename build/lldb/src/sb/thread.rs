use crate::strings::{get_cstring, get_str};
use crate::{
    debug_descr, IsValid, SBError, SBFileSpec, SBFrame, SBIterator, SBProcess, SBValue, ThreadID,
};
...
impl SBThread {
    ...
    pub fn name(&self) -> Option<&str> {
        let ptr = cpp!(unsafe [self as "SBThread*"] -> *const ffi::c_char as "const char*" {
            return self->GetName();
        });
        if ptr.is_null() {
            None
    ...
    pub fn frames(&self) -> impl Iterator<Item = SBFrame> + '_ {
