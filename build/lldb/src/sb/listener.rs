...
use crate::{IsValid, SBDebugger, SBEvent};
...
impl SBListener {
    ...
    // pub fn start_listening_for_events(&self, debugger: &SBDebugger, event_mask: u32) -> u32 {
    //     cpp!(unsafe [self as "SBListener*", debugger as "SBDebugger*", event_mask as "uint32_t"] -> u32 as "uint32_t" {
    //         return self->StartListeningForEvents(*debugger, event_mask);
    //     })
    // }
    // 
    // pub fn stop_listening_for_events(&self, debugger: &SBDebugger, event_mask: u32) -> bool {
    //     cpp!(unsafe [self as "SBListener*", debugger as "SBDebugger*", event_mask as "uint32_t"] -> bool as "bool" {
    //         return self->StopListeningForEvents(*debugger, event_mask);
    //     })
    // }
}
