use crate::strings::with_cstr;
use crate::{
    IsValid, SBCommandReturnObject, SBDebugger, SBError, SBExecutionContext, SBStringList,
};
...
impl SBCommandInterpreter {
    ...
    pub fn handle_command(&self, command: &str, add_to_history: bool) -> Result<CString, SBError> {
        let rtn = SBCommandReturnObject::new();

        let status = with_cstr(command, |command| {
            let ref_rtn = &rtn;

            cpp!(unsafe [self as "SBCommandInterpreter*", command as "const char*",
                         ref_rtn as "SBCommandReturnObject*", add_to_history as "bool"] -> ReturnStatus as "ReturnStatus" {
                return self->HandleCommand(command, *ref_rtn, add_to_history);
            })
        });

        if rtn.succeeded() {
            Ok(rtn.output().to_owned())
        } else {
            let err = SBError::new();
            err.set_error_string(rtn.output().to_str().unwrap());
            Err(err)
        }
    }

    pub fn handle_command_with_result(
        &self,
        command: &str,
    ...
    pub fn handle_command_with_context(
        &self,
        command: &str,
        context: &mut SBExecutionContext,
        add_to_history: bool,
        ...
        with_cstr(command, |command| {
            ...
            cpp!(unsafe [self as "SBCommandInterpreter*", command as "const char*", context as "SBExecutionContext*",
                         ref_rtn as "SBCommandReturnObject*", add_to_history as "bool"] -> ReturnStatus as "ReturnStatus" {
                return self->HandleCommand(command, *context, *ref_rtn, add_to_history);
            })
        });
    ...
    //     with_cstr(command, |command| {
    //         cpp!(unsafe [self as "SBCommandInterpreter*", command as "const char*", context as "SBExecutionContext*",
    //                      rtn as "SBCommandReturnObject*", add_to_history as "bool"] -> ReturnStatus as "ReturnStatus" {
    //             return self->HandleCommand(command, *context, *rtn, add_to_history);
    //         })
    //     });
    //
    //     println!("here2");
    ...
    pub fn handle_completions(
        ...
        unsafe {
            ...
            let (first_match, max_matches) = match range {
                None => (0, -1),
                Some((first_match, max_matches)) => {
                    (first_match as ffi::c_int, max_matches as ffi::c_int)
                }
            };
