...
use crate::{debug_descr, IsValid, SBAddress, SBCompileUnit, SBFileSpec, SBIterator, SBSymbol};
...
impl SBModule {
    pub fn uuid_string(&self) -> Option<&str> {
        let ptr = cpp!(unsafe [self as "SBModule*"] -> *const ffi::c_char as "const char*" {
    ...
    pub fn symbols(&self) -> impl Iterator<Item = SBSymbol> + '_ {
    ...
    pub fn compile_units(&self) -> impl Iterator<Item = SBCompileUnit> + '_ {
        SBIterator::new(self.num_compile_units(), move |index| {
            self.compile_unit_at_index(index)
        })
    }
}
