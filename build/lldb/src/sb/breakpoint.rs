...
use crate::{
    debug_descr, BreakpointID, IsValid, SBBreakpointLocation, SBIterator, SBProcess, SBThread,
};
...
type Callbacks = Mutex<
    HashMap<
        BreakpointID,
        Box<dyn FnMut(&SBProcess, &SBThread, &SBBreakpointLocation) -> bool + Send>,
    >,
>;

lazy_static::lazy_static! {
    static ref CALLBACKS: Callbacks  =
...
impl SBBreakpoint {
    ...
    pub fn locations(&self) -> impl Iterator<Item = SBBreakpointLocation> + '_ {
        SBIterator::new(self.num_locations(), move |index| {
            self.location_at_index(index)
        })
    }
    pub fn condition(&self) -> Option<&str> {
        let ptr = cpp!(unsafe [self as "SBBreakpoint*"] -> *const ffi::c_char as "const char*" {
        ...
        F: FnMut(&SBProcess, &SBThread, &SBBreakpointLocation) -> bool + Send + 'static,
    {
        ...
        let cb = callback_thunk as *const ffi::c_void;
