...
#[repr(C, align(128))]
pub struct Stack([u8; (Self::GUARD_PAGES + Self::SIZE_PAGES) * PAGE_SIZE]);
...
impl Stack {
    pub const SIZE_PAGES: usize = 16;
    pub const GUARD_PAGES: usize = 8;
    pub const FILL_PATTERN: u64 = 0xACE0BACE;

    pub const ZERO: Self = Self([0; (Self::GUARD_PAGES + Self::SIZE_PAGES) * PAGE_SIZE]);

    pub fn region(&self) -> RangeInclusive<PhysicalAddress> {
        let start = unsafe {
            PhysicalAddress::new(self.0.as_ptr() as usize).add(Self::GUARD_PAGES * PAGE_SIZE)
        };
        start..=start.add(Self::SIZE_PAGES * PAGE_SIZE)
    }
    ...
    pub fn usage(&self) -> StackUsage {
        ...
        StackUsage {
            ...
            total: Self::SIZE_PAGES * PAGE_SIZE,
