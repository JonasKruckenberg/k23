...
impl<M: Mode> Table<M> {
    ...
    pub unsafe fn new(addr: PhysicalAddress, level: usize) -> Table<M> {
        Self {
            level,
            addr,
            _m: PhantomData,
    ...
    pub fn entry(&self, index: usize) -> &Entry<M> {
        ...
        let ptr = self.addr.add(index * mem::size_of::<Entry<M>>()).as_raw() as *const Entry<M>;
        unsafe { &*ptr }
    }
    ...
    fn debug_print_table_inner(&self, acc: VirtualAddress) -> crate::Result<()> {
        ...
        for i in 0..M::PAGE_TABLE_ENTRIES {
            ...
            if M::entry_is_leaf(entry) {
                ...
                unsafe {
                    Self::new(entry.get_address(), self.level - 1).debug_print_table_inner(virt)?;
                }
            }
        }
...
mod test {
    ...
    use crate::{EmulateArch, PhysicalAddress};
    ...
    fn index_of_virt() {
        ...
        let table: Table<EmulateArch> = unsafe { Table::new(PhysicalAddress(0), 2) };
        assert_eq!(table.index_of_virt(virt), 511);

        let table: Table<EmulateArch> = unsafe { Table::new(PhysicalAddress(0), 1) };
        assert_eq!(table.index_of_virt(virt), 510);

        let table: Table<EmulateArch> = unsafe { Table::new(PhysicalAddress(0), 0) };
