...
// const MEGA_PAGE_SIZE: usize = 2 * MIB;
// const GIGA_PAGE_SIZE: usize = 1 * GIB;
...
impl<'a, M: Mode> Mapper<'a, M> {
    ...
    pub fn map_with_flush(
        ...
        debug_assert!(level < M::PAGE_TABLE_LEVELS);
        debug_assert!(
    ...
    fn unmap_inner(
        ...
        if level == 0 {
            ...
            let mut subtable = unsafe { Table::new(entry.get_address(), level - 1) };
