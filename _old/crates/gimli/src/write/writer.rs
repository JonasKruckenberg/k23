...
pub trait Writer {
    ...
    fn write_uleb128(&mut self, val: u64) -> Result<()> {
        ...
        let mut writeable = BorrowedBuf::from(bytes.as_mut_slice());
        let len = leb128::write::unsigned(writeable.unfilled(), val).unwrap();
    ...
    fn write_sleb128(&mut self, val: i64) -> Result<()> {
        ...
        // bytes is long enough so this will never fail.
        let mut writeable = BorrowedBuf::from(bytes.as_mut_slice());
        let len = leb128::write::signed(writeable.unfilled(), val).unwrap();
        self.write(&bytes[..len])
    }
