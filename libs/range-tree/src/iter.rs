...
pub(crate) struct RawIter<I: Idx, V> {
    /// Current leaf node.
    pub(crate) node: NodeRef<marker::Leaf<V>>,
...
impl<I: Idx, V> Clone for RawIter<I, V> {
    fn clone(&self) -> Self {
        Self { node: self.node.clone(), pos: self.pos.clone() }
    }
}

impl<I: Idx, V> RawIter<I, V> {
    ...
    #[inline]
    unsafe fn is_end(&self, leaf_pool: &NodePool<I, marker::Leaf<V>>) -> bool {
    ...
    /// `leaf_pool` must point to the `NodePool` for leaf nodes in the tree.
    #[inline]
    unsafe fn next_key(&self, leaf_pool: &NodePool<I, marker::Leaf<V>>) -> I::Raw {
    ...
    /// `leaf_pool` must point to the `NodePool` for leaf nodes in the tree.
    #[inline]
    pub(crate) unsafe fn next(&mut self, leaf_pool: &NodePool<I, marker::Leaf<V>>) -> Option<(ops::Range<I>, NonNull<V>)> {
        // Get the current element that will be returned.
        let pivot = unsafe { I::from_raw(self.node.pivot(self.pos, leaf_pool))? };
        let start = unsafe { I::from_raw(self.node.start(self.pos, leaf_pool).assume_init_read()).unwrap() };
        ...
        Some((start..pivot, value.cast()))
    }
}

/// An iterator over the keys of a [`BTree`].
pub struct Ranges<'a, I: Idx, V, A: Allocator> {
    raw: RawIter<I, V>,
    btree: &'a RangeTree<I, V, A>,
}

impl<'a, I: Idx, V, A: Allocator> Iterator for Ranges<'a, I, V, A> {
...
impl<'a, I: Idx, V, A: Allocator> FusedIterator for Ranges<'a, I, V, A> {}

impl<'a, I: Idx, V, A: Allocator> Clone for Ranges<'a, I, V, A> {
    fn clone(&self) -> Self {
        Self {
            raw: self.raw.clone(),
            btree: self.btree,
...
pub struct Values<'a, I: Idx, V, A: Allocator> {
    raw: RawIter<I, V>,
    btree: &'a RangeTree<I, V, A>,
}

impl<'a, I: Idx, V, A: Allocator> Iterator for Values<'a, I, V, A> {
    type Item = &'a V;
...
impl<'a, I: Idx, V, A: Allocator> FusedIterator for Values<'a, I, V, A> {}

impl<'a, I: Idx, V, A: Allocator> Clone for Values<'a, I, V, A> {
    fn clone(&self) -> Self {
        Self {
            raw: self.raw.clone(),
            btree: self.btree,
...
pub struct ValuesMut<'a, I: Idx, V, A: Allocator> {
    raw: RawIter<I, V>,
    btree: &'a mut RangeTree<I, V, A>,
}

impl<'a, I: Idx, V, A: Allocator> Iterator for ValuesMut<'a, I, V, A> {
    type Item = &'a mut V;
...
impl<'a, I: Idx, V, A: Allocator> FusedIterator for ValuesMut<'a, I, V, A> {}
...
impl<I: Idx, V, A: Allocator> RangeTree<I, V, A> {
    ...
    pub(crate) fn raw_iter(&self) -> RawIter<I, V> {
        ...
        // never deleted.
        RawIter { node: NodeRef::ZERO, pos: NodePos::ZERO }
    }
