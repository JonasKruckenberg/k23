...
use crate::node::{NodePos, NodeRef, pos};
    ...
    RawCursor<I, V, A, Ref>
{
    ...
    #[inline]
    fn entry(&self) -> Option<(ops::Range<I>, NonNull<V>)> {
        let (node, pos) = self.stack[Height::LEAF];
        let pivot = unsafe { I::Int::from_raw(node.pivot(pos, &self.tree.leaf))? };
        let start = unsafe {
            I::Int::from_raw(node.start(pos, &self.tree.leaf).assume_init_read()).unwrap()
        };
...
impl<I: RangeTreeIndex, V, A: Allocator> RawCursor<I, V, A, &'_ mut RangeTree<I, V, A>> {
    ...
    fn replace(&mut self, range: ops::Range<I>, value: V) -> (ops::Range<I>, V) {
        ...
        let old_start = unsafe {
            I::Int::from_raw(mem::replace(
                node.start_mut(pos, &mut self.tree.leaf).assume_init_mut(),
                range.start.to_int().to_raw(),
            ))
            .unwrap_unchecked()
        };
        ...
        (I::from_int(old_start)..I::from_int(old_pivot), old_value)
    }
}
...
impl<'a, I: RangeTreeIndex, V, A: Allocator> Cursor<'a, I, V, A> {
    ...
    pub fn entry(&self) -> Option<(ops::Range<I>, &V)> {
        self.raw
            .entry()
            .map(|(range, value)| unsafe { (range, value.as_ref()) })
    }
...
impl<'a, I: RangeTreeIndex, V, A: Allocator> CursorMut<'a, I, V, A> {
    ...
    pub fn entry_mut(&mut self) -> Option<(ops::Range<I>, &mut V)> {
        self.raw
            .entry()
            .map(|(range, mut value)| unsafe { (range, value.as_mut()) })
    }
