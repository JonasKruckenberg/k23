...
impl NodeRef {
    #[inline]
    pub(crate) unsafe fn payloads_ptr<I: RangeTreeInteger, V>(
        self,
        pool: &NodePool<I, marker::Leaf<V>>,
    ) -> NonNull<MaybeUninit<LeafNodePayload<I, V>>> {
        #[cfg(debug_assertions)]
        self.assert_valid(pool);

        let LeafNodeLayout { payloads_offset: values_offset, .. } =  const { leaf_node_layout::<I, V>() };
        unsafe {
            let ptr = pool.ptr.byte_add(self.0 as usize);
            ptr.byte_add(values_offset).cast::<MaybeUninit<LeafNodePayload<I, V>>>()
        }
    }
    ...
    unsafe fn next_leaf_ptr<I: RangeTreeInteger, V>(
        ...
        let LeafNodeLayout { next_leaf_offset, .. } = const { leaf_node_layout::<I, V>() };
        unsafe {
            let ptr = pool.ptr.byte_add(self.0 as usize);
    ...
    #[inline]
    pub(crate) unsafe fn payload_mut<I: RangeTreeInteger, V>(
        self,
        pos: NodePos<I>,
        pool: &mut NodePool<I, marker::Leaf<V>>,
    ) -> &mut MaybeUninit<LeafNodePayload<I, V>> {
        unsafe { self.payloads_ptr(pool).add(pos.index()).as_mut() }
    }

    #[inline]
    pub(crate) unsafe fn insert_payload<I: RangeTreeInteger, V>(
        self,
        payload: LeafNodePayload<I, V>,
        pos: NodePos<I>,
        node_size: usize,
        pool: &mut NodePool<I, marker::Leaf<V>>,
    ) {
        ...
        debug_assert!(node_size > pos.index());
        unsafe {
            let ptr = self.payloads_ptr(pool).add(pos.index());
            let count = node_size - pos.index() - 1;
            ptr.copy_to(ptr.add(1), count);
            ptr.write(MaybeUninit::new(payload));
        }
    }
    ...
    pub(crate) unsafe fn leaf_split_into<I: RangeTreeInteger, V>(
        ...
        unsafe {
            ...
            self.pivots_ptr(pool)
                .add(I::B / 2)
                .copy_to_nonoverlapping(dest.0.pivots_ptr(pool), I::B / 2);
            self.payloads_ptr(pool)
                .add(I::B / 2)
                .copy_to_nonoverlapping(dest.0.payloads_ptr(pool), I::B / 2);
...
pub(crate) struct LeafNodePayload<I: RangeTreeInteger, V> {
    pub start: I,
    pub value: V,
}
#[derive(Debug)]
pub(crate) struct LeafNodeLayout {
    pub(crate) layout: Layout,
    pub(crate) payloads_offset: usize,
    pub(crate) next_leaf_offset: usize,
}
pub(crate) const fn leaf_node_layout<I: RangeTreeInteger, V>() -> LeafNodeLayout {
    let layout = Layout::new::<I::Pivots>();

    let Ok(payloads) = Layout::array::<LeafNodePayload<I, V>>(I::B - 1) else {
        panic!("Could not calculate node layout");
    };
    let Ok((layout, payloads_offset)) = layout.extend(payloads) else {
        panic!("Could not calculate node layout");
    };
...
impl<I: RangeTreeInteger, V> NodePool<I, marker::Leaf<V>> {
    ...
    pub(crate) unsafe fn alloc_node(
        ...
        let LeafNodeLayout { layout, .. } = const { leaf_node_layout::<I, V>() };

        unsafe { self.alloc_node_inner(layout, allocator) }
    }
    ...
    pub(crate) unsafe fn clear_and_free(&mut self, allocator: &impl Allocator) {
        let LeafNodeLayout { layout, .. } =  const { leaf_node_layout::<I, V>() };
        unsafe { self.clear_and_free_inner(layout, allocator) }
    }
}
...
mod tests {
    ...
    fn layout() {
        ...
        let LeafNodeLayout { layout, payloads_offset: values_offset, next_leaf_offset } =
            const { leaf_node_layout::<NonMaxU64, usize>() };

        let size_pivots = size_of::<u64>() * NonMaxU64::B;
        let size_values = size_of::<(u64, usize)>() * (NonMaxU64::B - 1);

        assert!(layout.align() >= align_of::<NodeRef>());
        assert!(
            layout.size() >= size_pivots + size_values + (size_of::<NodeRef>()), // next leaf
            "leaf node layout too small! must be at least "
        );
        assert_eq!(values_offset, size_pivots);
        assert_eq!(next_leaf_offset, size_pivots + size_values);
    }
}
