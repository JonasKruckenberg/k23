    ...
    C: cfg::Config,
{
    ...
    pub(crate) fn take_remote(&self, idx: usize) -> Option<T> {
        ...
        log::trace!("-> take_remote {:?}; page {:?}", addr, page_index);
    ...
    pub(crate) fn iter(&self) -> slice::Iter<'_, page::Shared<Option<T>, C>> {
        self.shared.iter()
    }
}
    ...
    C: cfg::Config,
{
    pub(crate) fn init_with<U>(
        ...
        for (page_idx, page) in self.shared.iter().enumerate() {
            ...
            log::trace!("-> page {}; {:?}; {:?}", page_idx, local, page);
...
impl<T, C: cfg::Config> Ptr<T, C> {
    ...
    fn load(&self, order: Ordering) -> Option<&Shard<T, C>> {
        ...
        log::trace!("---> loaded={:p} (order={:?})", ptr, order);
        if ptr.is_null() {
            log::trace!("---> null");
            return None;
        }
