...
//!
//! "Forked" from [sharded-slab](https://github.com/hawkw/sharded-slab) (MIT licensed) with changes
...
impl<T, C: cfg::Config> Slab<T, C> {
    ...
    pub fn get(&self, key: usize) -> Option<Entry<'_, T, C>> {
        ...
        log::trace!("get {:?}; current={:?}", tid, Tid::<C>::current());
        let shard = self.shards.get(tid.as_usize())?;
    ...
    pub fn get_owned(self: Arc<Self>, key: usize) -> Option<OwnedEntry<T, C>> {
        ...
        log::trace!("get_owned {:?}; current={:?}", tid, Tid::<C>::current());
        let shard = self.shards.get(tid.as_usize())?;
        shard.with_slot(key, |slot| {
            let inner = slot.get(C::unpack_gen(key))?;
            let value = ptr::NonNull::from(slot.value().as_ref().unwrap());
...
impl<T, C: cfg::Config> core::ops::Deref for Entry<'_, T, C> {
    type Target = T;
...
impl<T, C> core::ops::Deref for OwnedEntry<T, C>
where
    C: cfg::Config,
{
    type Target = T;
