use crate::frame_alloc::{bump::BumpAllocator, zero_frames, FrameAllocator, FrameUsage};
use crate::{phys_to_virt, AddressRangeExt, Arch, Error, PhysicalAddress, VirtualAddress};
use core::fmt;
...
where
    A: Arch,
{
    const NUM_ENTRIES: usize = A::PAGE_SIZE / size_of::<TableEntry<A>>();

    fn entries_mut(&mut self) -> &mut [TableEntry<A>] {
        unsafe {
    ...
    pub fn new(mut bump_allocator: BumpAllocator<A>) -> crate::Result<Self> {
        ...
        log::debug!(
            ...
            table_virt..table_virt.add(A::PAGE_SIZE)
        );
        ...
        for entry in this.entries_mut() {
            ...
            if usage_map_pages > 0 {
                ...
                zero_frames(addr.as_raw() as *mut u64, usage_map_pages * A::PAGE_SIZE);
            }
...
impl<A> FrameAllocator<A> for BitMapAllocator<A>
where
    A: Arch,
{
    ...
    fn deallocate_frames(&mut self, base: PhysicalAddress, num_frames: usize) -> crate::Result<()> {
        ...
        for entry in self.entries_mut() {
            if base >= entry.region.start && base.add(num_frames * A::PAGE_SIZE) <= entry.region.end
            {
                let start_page = (base.0 - entry.region.start.0) >> A::PAGE_SHIFT;
