...
pub(super) struct Layout<'a> {
    ...
    pub(super) files: HashMap<blake3::Hash, FileNode<'a>>,
...
impl<'a> Layout<'a> {
            ...
            for (name, entry) in dir.entries {
                match entry {
                    ...
                    Entry::File(File {
                        ...
                        files
                            .entry(content_hash)
                            .or_insert(FileNode::new(name.clone(), source));

                        // files are comparatively easy: we just need to make sure to
                        // update our parent so it knows where this file is
                        dirs[parent_idx as usize].attach_file(name, content_hash);
                    }
                }
            }
        }
    ...
    pub(super) fn assign_lbas(
        ...
        // to the LBA of the corresponding file in the directory tree.
    ...
    fn find_file(&self, path: &str) -> Option<&FileNode<'a>> {
        ...
        loop {
            ...
            if parts.peek().is_none() {
                ...
                let (_, DirNodeEntry::File(file_id)) = &self.dirs[dir_idx].sorted_entry_ids[pos]
                else {
                    panic!("todo not a file");
                };

                return Some(self.files.get(file_id).unwrap());
            }
...
enum DirNodeEntry {
    ...
    ChildDirectory(u16),
    // the content hash of the file. we look this up in the global hashmap of files
    File(blake3::Hash),
}
...
impl<'a> DirNode<'a> {
    ...
    fn attach_file(&mut self, name: Cow<'a, str>, child_id: blake3::Hash) {
        self.sorted_entry_ids
            .push((name, DirNodeEntry::File(child_id)));
    }
    ...
    fn files<'l>(
        &self,
        all_files: &'l HashMap<blake3::Hash, FileNode<'a>>,
    ) -> impl Iterator<Item = &'l FileNode<'a>> {
        self.sorted_entry_ids.iter().filter_map(|(_, e)| {
            if let DirNodeEntry::File(idx) = e {
                Some(all_files.get(idx).unwrap())
            } else {
                None
            }
        })
...
fn serialize_dir_extent(
    ...
    dir: &DirNode<'_>,
    dirs: &[DirNode<'_>],
    files: &HashMap<blake3::Hash, FileNode<'_>>,
) -> io::Result<()> {
