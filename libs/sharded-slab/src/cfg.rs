...
pub(crate) trait CfgPrivate: Config {
    ...
    fn validate() {
        ...
        assert!(
            Self::USED_BITS <= usize::BITS as usize,
            "invalid Config: {:#?}\ntotal number of bits per index is too large to fit in a word!",
            Self::debug()
        );

        assert!(
            usize::BITS as usize - Self::USED_BITS >= Self::RESERVED_BITS,
...
impl Config for DefaultConfig {
    ...
    const MAX_PAGES: usize = usize::BITS as usize / 2;
}
...
impl<C: Config> fmt::Debug for DebugConfig<C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct(core::any::type_name::<C>())
