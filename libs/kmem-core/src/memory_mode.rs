...
impl MemoryMode {
    ...
    pub const fn canonicalize(&self, address: VirtualAddress) -> VirtualAddress {
        let shift = usize::BITS - self.va_bits;
        VirtualAddress::new((((address.get() as isize) << shift) >> shift) as usize)
    }
}
...
impl<A: Arch> MemoryModeBuilder<A, HasLevels, HasPhysmap> {
    pub const fn finish(self) -> MemoryMode {
        ...
        result.va_bits = result.levels()[0].entries().ilog2() + result.levels()[0].index_shift;
