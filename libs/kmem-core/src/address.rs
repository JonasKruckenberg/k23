...
impl VirtualAddress {
    ...
    pub const fn canonicalize(&self, memory_mode: &MemoryMode) -> Self {
        let shift = usize::BITS - memory_mode.virtual_address_bits() as u32;
        Self::new((((self.get() as isize) << shift) >> shift) as usize)
    }

    pub fn is_canonical(&self, memory_mode: &MemoryMode) -> bool {
        let mask = !((1 << (memory_mode.virtual_address_bits())) - 1);
