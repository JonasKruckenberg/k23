...
use super::{
    ...
    UnwindException, UnwindReasonCode,
};
use crate::arch;
use crate::unwinder::{UnwindAction, UnwindContext};
use core::ffi::c_int;
...
unsafe fn rust_eh_personality(
    ...
    let lsda = unwinder::_Unwind_GetLanguageSpecificData(unwind_ctx);
    if lsda.is_null() {
    ...
    if actions.contains(UnwindAction::SEARCH_PHASE) {
        ...
        match eh_action {
            ...
            EHAction::Cleanup(lpad) | EHAction::Catch(lpad) => {
                unwinder::_Unwind_SetGR(
                    unwind_ctx,
                    i32::from(arch::UNWIND_DATA_REG.0 .0),
                    exception as usize,
                );
                unwinder::_Unwind_SetGR(unwind_ctx, i32::from(arch::UNWIND_DATA_REG.1 .0), 0);
                unwinder::_Unwind_SetIP(unwind_ctx, lpad);
...
fn find_eh_action(
    ...
    let func_start = unwinder::_Unwind_GetRegionStart(unwind_ctx);
    let mut ip_before_instr = 0;
    let ip = unwinder::_Unwind_GetIPInfo(unwind_ctx, &mut ip_before_instr);
...
fn parse_encoded_pointer(
    ...
    let base = match encoding.application() {
        ...
        constants::DW_EH_PE_textrel => unwinder::_Unwind_GetTextRelBase(unwind_ctx) as u64,
        constants::DW_EH_PE_datarel => unwinder::_Unwind_GetDataRelBase(unwind_ctx) as u64,
        constants::DW_EH_PE_funcrel => unwinder::_Unwind_GetRegionStart(unwind_ctx) as u64,
