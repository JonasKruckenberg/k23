...
mod tests {
    ...
    //     let mut buf = [0; 1];
    //     let mut writable = BorrowedBuf::from(buf.as_mut_slice());
    //
    //     match write::unsigned(writable.unfilled(), 128) {
    ...
    fn dogfood_signed() {
        fn inner(i: i64) {
            ...
            {
                let mut writable = BorrowedBuf::from(buf.as_mut_slice());
                write::signed(writable.unfilled(), i).expect("Should write signed number");
            }
    ...
    fn dogfood_unsigned() {
        for i in 0..1025 {
            ...
            {
                let mut writable = BorrowedBuf::from(buf.as_mut_slice());
                write::unsigned(writable.unfilled(), i).expect("Should write signed number");
            }
