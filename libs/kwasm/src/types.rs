...
use crate::wasm::{
    WasmCompositeType, WasmCompositeTypeInner, WasmEntityType, WasmFieldType, WasmFuncType,
    WasmGlobalType, WasmHeapType, WasmHeapTypeInner, WasmMemoryType, WasmRefType, WasmStorageType,
    WasmSubType, WasmTableType, WasmTagType, WasmValType,
};
...
impl StructType {
    ...
    pub(crate) fn comes_from_same_engine(&self, other: &Engine) -> bool {
...
impl TableType {
    pub(crate) fn from_wasm(engine: &Engine, table: &WasmTableType) -> Self {
        Self {
            element: RefType::from_wasm_type(engine, &table.element_type),
            ty: table.clone(),
        }
    }
}
...
impl MemoryType {
    pub(crate) fn from_wasm(memory: &WasmMemoryType) -> Self {
...
impl GlobalType {
    ...
    pub(crate) fn to_wasm(&self) -> WasmGlobalType {
        WasmGlobalType {
    ...
    pub(crate) fn from_wasm(engine: &Engine, ty: &WasmGlobalType) -> Self {
...
impl TagType {
    pub(crate) fn from_wasm(engine: &Engine, tag: &WasmTagType) -> Self {
        Self {
            ty: FuncType::from_shared_type_index(
                engine,
                tag.signature
                    .as_engine_type_index()
                    .expect("function signature index not canonicalized for runtime usage"),
            ),
        }
    }
}
