...
use crate::vm::const_eval::{ConstEvalContext, ConstExprEvaluator};
use crate::vm::provenance::{VmPtr, VmSafe};
use crate::vm::table::{TableElement, TableElementType};
use crate::vm::vmcontext::{VMBuiltinFunctionsArray, VMCONTEXT_MAGIC};
use crate::vm::vmcontext_shape::{StaticVMContextShape, VMContextShape};
use crate::vm::{
    Export, ExportedFunction, ExportedGlobal, ExportedMemory, ExportedTable, ExportedTag, Imports,
    Memory, Table, VMContext, VMFuncRef, VMFunctionImport, VMGlobalDefinition, VMGlobalImport,
    VMMemoryDefinition, VMMemoryImport, VMStoreContext, VMTableDefinition, VMTableImport,
    VMTagDefinition, VMTagImport,
};
use crate::wasm::{
    MemoryInitializer, TableInitialValue, TableSegmentElements, TranslatedModule,
    WasmHeapTypeInner, WasmIndexType,
};
use crate::{Module, TrapKind};
...
pub struct Instance {
    module: Module,
    pub(crate) memories: PrimaryMap<DefinedMemoryIndex, Memory>,
    pub(crate) tables: PrimaryMap<DefinedTableIndex, Table>,
...
impl InstanceHandle {
    ...
    /// panic.
    pub const fn null() -> InstanceHandle {
    ...
    pub fn initialize(
        ...
        unsafe {
            self.instance_mut()
                .initialize_vmctx(store.as_mut(), imports, module);

            if !is_bulk_memory {
                // Safety: see? we called `initialize_vmctx` before calling `check_init_bounds`!
                check_init_bounds(store.as_mut(), self.instance_mut(), module)?;
            }

            let mut ctx = ConstEvalContext::new(self.instance.unwrap().as_mut());
            self.instance_mut()
                .initialize_tables(store.as_mut(), &mut ctx, const_eval, module)?;
            self.instance_mut().initialize_memories(
                store.as_mut(),
                &mut ctx,
                const_eval,
                module,
                ...
                unsafe {
                    f.debug_struct("VMContext")
                        .field(
                            ...
                            &*self
                                .data
                                .vmctx_plus_offset::<u32>(StaticVMContextShape.vmctx_magic()),
                        )
                        .field(
                            "vm_store_context",
                            &*self
                                ...
                                .vmctx_plus_offset::<Option<VmPtr<VMStoreContext>>>(
                                    StaticVMContextShape.vmctx_store_context(),
                                ),
                        )
                        .field(
                            "builtin_functions",
                            &*self
                                ...
                                .vmctx_plus_offset::<VmPtr<VMBuiltinFunctionsArray>>(
                                    StaticVMContextShape.vmctx_builtin_functions(),
                                ),
                        )
                        // .field(
                        //     "callee",
                        ...
                        //             StaticVMContextShape.vmctx_callee(),
                        //         ),
                        // )
                        .field(
                            "epoch_ptr",
                            &*self.data.vmctx_plus_offset::<Option<VmPtr<AtomicU64>>>(
                                StaticVMContextShape.vmctx_epoch_ptr(),
                            ),
                        )
                        .field(
                            "gc_heap_base",
                            &*self.data.vmctx_plus_offset::<Option<VmPtr<u8>>>(
                                StaticVMContextShape.vmctx_gc_heap_base(),
                            ),
                        )
                        .field(
                            "gc_heap_bound",
                            &*self.data.vmctx_plus_offset::<usize>(
                                StaticVMContextShape.vmctx_gc_heap_bound(),
                            ),
                        )
                        .field(
                            "gc_heap_data",
                            &*self.data.vmctx_plus_offset::<Option<VmPtr<u8>>>(
                                StaticVMContextShape.vmctx_gc_heap_data(),
                            ),
                        )
                        .field(
                            "type_ids",
                            &*self.data.vmctx_plus_offset::<VmPtr<VMSharedTypeIndex>>(
                                StaticVMContextShape.vmctx_type_ids_array(),
                            ),
                        )
                        .field(
                            "imported_memories",
...
impl Instance {
    ...
    pub fn alloc_layout(vmctx_shape: &VMContextShape) -> Layout {
        let size = size_of::<Self>()
            .checked_add(usize::try_from(vmctx_shape.size_of_vmctx()).unwrap())
            .unwrap();
    ...
    pub fn vmshape(&self) -> &VMContextShape {
        self.module.vmctx_shape()
    }
    ...
    pub fn get_exported_memory(&mut self, index: MemoryIndex) -> ExportedMemory {
        let (definition, vmctx) =
            if let Some(def_index) = self.translated_module().defined_memory_index(index) {
                (self.memory_ptr(def_index), self.vmctx())
            } else {
        ...
        match elements {
            ...
            TableSegmentElements::Expressions(exprs) => {
                let exprs = exprs
                    ...
                    .and_then(|s| s.get(..len))
                    .ok_or(TrapKind::TableOutOfBounds)?;
                let heap_top_ty = self.translated_module().tables[table_index]
                    .element_type
                    .heap_type
                    .top();
                assert!(!heap_top_ty.shared);

                // Safety: the implementation promises that vmctx is correctly initialized
                let mut context = unsafe { ConstEvalContext::new(self) };

                match heap_top_ty.inner {
                    WasmHeapTypeInner::Func => table.init_func(
                        dst,
                        exprs.iter().map(|expr| {
                            NonNull::new(
                                const_eval
                                    .eval(store.as_mut(), &mut context, expr)
                    ...
                    WasmHeapTypeInner::Extern | WasmHeapTypeInner::Any => todo!("gc proposal"),
                    WasmHeapTypeInner::Exn => todo!("exception-handling proposal"),
                    WasmHeapTypeInner::Cont => todo!("continuation proposal"),
                    _ => unreachable!(),
                }
            }
        }
    ...
    // unsafe fn set_gc_heap(&mut self, gc_store: Option<Pin<&mut StoreOpaque>>) {
    ...
    pub fn vm_store_context(&mut self) -> NonNull<Option<VmPtr<VMStoreContext>>> {
        // Safety: the implementation promises that vmctx is correctly initialized
        unsafe {
            NonNull::new(self.vmctx_plus_offset_mut(StaticVMContextShape.vmctx_store_context()))
                .unwrap()
        }
    }
    ...
    pub fn epoch_ptr(&mut self) -> NonNull<Option<VmPtr<AtomicU64>>> {
        // Safety: the implementation promises that vmctx is correctly initialized
        unsafe {
            NonNull::new(self.vmctx_plus_offset_mut(StaticVMContextShape.vmctx_epoch_ptr()))
                .unwrap()
        }
    }
    ...
    pub fn gc_heap_base(&mut self) -> NonNull<Option<VmPtr<u8>>> {
        // Safety: the implementation promises that vmctx is correctly initialized
        unsafe {
            NonNull::new(self.vmctx_plus_offset_mut(StaticVMContextShape.vmctx_gc_heap_base()))
                .unwrap()
        }
    }
    ...
    pub fn gc_heap_bound(&mut self) -> NonNull<usize> {
        // Safety: the implementation promises that vmctx is correctly initialized
        unsafe {
            NonNull::new(self.vmctx_plus_offset_mut(StaticVMContextShape.vmctx_gc_heap_bound()))
                .unwrap()
        }
    }
    ...
    pub fn gc_heap_data(&mut self) -> NonNull<Option<VmPtr<u8>>> {
        // Safety: the implementation promises that vmctx is correctly initialized
        unsafe {
            NonNull::new(self.vmctx_plus_offset_mut(StaticVMContextShape.vmctx_gc_heap_data()))
                .unwrap()
        }
    }
    ...
    fn memory_ptr(&mut self, index: DefinedMemoryIndex) -> NonNull<VMMemoryDefinition> {
        // Safety: the implementation promises that vmctx is correctly initialized
        let ptr = unsafe {
            *self.vmctx_plus_offset::<VmPtr<_>>(self.vmshape().vmctx_vmmemory_definition(index))
        };
        ptr.as_non_null()
    }
    ...
    unsafe fn initialize_vmctx(
        ...
        let vmshape = module.vmctx_shape();
        ...
        unsafe {
            ...
            self.set_store(Some(NonNull::from(Pin::into_inner_unchecked(store))));
                    ...
                    })
            {
                let ptr = self.vmctx_plus_offset_mut::<VmPtr<VMMemoryDefinition>>(
                    vmshape.vmctx_vmmemory_definition(def_index),
                );
                ...
                if desc.shared {
                    ...
                    let ptr = self.vmctx_plus_offset_mut::<VMMemoryDefinition>(
                        vmshape.vmctx_vmmemory_definition(def_index),
                    );

                    ptr.write(self.memories[def_index].vmmemory_definition());
                }
            }
            ...
            for (index, _init) in &module.translated().global_initializers {
                self.global_ptr(index).write(VMGlobalDefinition::null());
            }
    ...
    unsafe fn initialize_globals(
        ...
        for (def_index, init) in &module.translated().global_initializers {
            ...
            unsafe {
                self.global_ptr(def_index)
                    .write(VMGlobalDefinition::from_vmval(store.as_mut(), ty, vmval)?);
            }
        }
    ...
    unsafe fn initialize_tables(
        ...
        for (def_index, init) in &module.translated().table_initializers.initial_values {
            match init {
                ...
                TableInitialValue::ConstExpr(expr) => {
                    ...
                    let heap_top_ty = self.translated_module().tables[index]
                        .element_type
                        .heap_type
                        .top();
                    assert!(!heap_top_ty.shared);
        ...
        for segment in &module.translated().table_initializers.segments {
            let start = const_eval
                .eval(store.as_mut(), ctx, &segment.offset)
                .expect("const expression should be valid");
    ...
    unsafe fn initialize_memories(
        ...
        for initializer in &module.translated().memory_initializers {
            let start: usize = {
                let vmval = const_eval
                    .eval(store.as_mut(), ctx, &initializer.offset)
                    .expect("const expression should be valid");

                match self.translated_module().memories[initializer.memory_index].index_type {
                    WasmIndexType::I32 => usize::try_from(vmval.get_u32()).unwrap(),
                    WasmIndexType::I64 => usize::try_from(vmval.get_u64()).unwrap(),
                }
            };
...
impl InstanceAndStore {
    ...
    #[inline]
    pub(crate) fn unpack_mut(&mut self) -> (&mut Instance, Pin<&mut StoreOpaque>) {
        // Safety: this is fine
        unsafe {
            let store = Pin::new_unchecked(self.instance.store.unwrap().as_mut());
...
unsafe fn check_init_bounds(
    ...
    // Safety: ensured by caller
    unsafe {
        check_table_init_bounds(store.as_mut(), instance, module)?;
    ...
    unsafe {
        ...
        for segment in &module.translated().table_initializers.segments {
            ...
            let start = const_evaluator
                .eval(store.as_mut(), &mut context, &segment.offset)
                .expect("const expression should be valid");
    ...
    unsafe {
        for init in initializers {
            let memory = instance
                ...
                .as_ref();
            let start = get_memory_init_start(store.as_mut(), init, instance)?;
    ...
    const_evaluator
        ...
        .map(
            |v| match instance.translated_module().memories[init.memory_index].index_type {
                WasmIndexType::I32 => v.get_u32().into(),
                WasmIndexType::I64 => v.get_u64(),
            },
        )
}
