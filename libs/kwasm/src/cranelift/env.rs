...
// use crate::wasm::{ModuleTypes, TranslatedModule, WasmMemoryType, WasmTableType};
use alloc::vec;
use alloc::vec::Vec;
use core::cmp;
...
use crate::cranelift::utils::index_type_to_ir_type;
...
use crate::trap::{TRAP_BAD_SIGNATURE, TRAP_INDIRECT_CALL_TO_NULL};
use crate::utils::{reference_type, u8_size_of, u32_offset_of, value_type, wasm_call_signature};
use crate::vm::{
    StaticVMContextShape, VMContextShape, VMFuncRef, VMFunctionImport, VMGlobalImport,
    VMMemoryDefinition, VMMemoryImport, VMTableDefinition, VMTableImport,
};
use crate::wasm::{
    ModuleTypes, TranslatedModule, WasmHeapType, WasmHeapTypeInner, WasmIndexType, WasmMemoryType,
    WasmRefType, WasmTableType, WasmValType, WasmparserTypeConverter,
};
...
impl<'module_env> TranslationEnvironment<'module_env> {
    pub(crate) fn new(
        ...
        let vmoffsets = VMContextShape::for_module(isa.pointer_bytes(), module);
    ...
    fn memory_ty(&self, memory_index: MemoryIndex) -> &WasmMemoryType {
        &self.module.memories[memory_index]
    }

    fn table_ty(&self, table_index: TableIndex) -> &WasmTableType {
    ...
    fn cast_index_to_i64(
        ...
        val: Value,
        index_type: WasmIndexType,
    ) -> Value {
        match index_type {
            WasmIndexType::I32 => pos.ins().uextend(I64, val),
            WasmIndexType::I64 => val,
        }
    }
...
impl TranslationEnvironment<'_> {
    ...
    pub fn make_memory(&mut self, func: &mut Function, index: MemoryIndex) -> CraneliftMemory {
        ...
        let (base, base_offset, ptr_memtype) = match self.module.defined_memory_index(index) {
            Some(_) if ty.shared => todo!("shared memory"),
            Some(def_index) => {
                let base_offset = self.vmshape.vmctx_vmmemory_definition(def_index)
        ...
        let (base_fact, memory_type) = if let Some(ptr_memtype) = ptr_memtype {
            ...
            let data_mt = func.create_memory_type(ir::MemoryTypeData::Memory {
                ...
                size: ty.max_size_based_on_index_type(),
            });
        ...
        let min_size = ty.minimum_byte_size().unwrap_or_else(|_| {
            ...
            debug_assert_eq!(ty.limits.min, 1 << 48i32);
            debug_assert_eq!(ty.page_size(), 1 << 16i32);
            u64::MAX
        });
        let max_size = ty.maximum_byte_size().ok();
        ...
        CraneliftMemory {
            ...
            min_size,
            max_size,
            bound: ty.max_size_based_on_index_type(),
            index_type: ty.index_type,
            offset_guard_size: 0, // TODO fix
    ...
    pub fn translate_memory_grow(
        ...
        let delta =
            self.cast_index_to_i64(&mut pos, delta, self.memory_ty(memory_index).index_type);
        ...
        let single_byte_pages = match self.memory_ty(memory_index).page_size_log2 {
            16 => false,
            0 => true,
        ...
        self.convert_pointer_to_index_type(
            pos,
            result,
            self.memory_ty(memory_index).index_type,
            single_byte_pages,
        )
    }
            ...
            Some(def_index) => {
                if is_shared {
                    ...
                    let offset = i32::try_from(
                        self.vmshape.vmctx_vmmemory_definition(def_index)
    ...
    pub fn translate_memory_copy(
        ...
        let dst = self.cast_index_to_i64(&mut pos, dst_pos, self.memory_ty(dst_index).index_type);
        let src = self.cast_index_to_i64(&mut pos, src_pos, self.memory_ty(src_index).index_type);
        ...
        // `dst`/`src` above)
        let len = if index_type_to_ir_type(self.memory_ty(dst_index).index_type) == I64
            && index_type_to_ir_type(self.memory_ty(src_index).index_type) == I64
        {
            len
        } else {
    ...
    pub fn translate_memory_fill(
        ...
        let vmctx = self.vmctx_val(&mut pos);
        let dst = self.cast_index_to_i64(&mut pos, dst, self.memory_ty(memory_index).index_type);
        let len = self.cast_index_to_i64(&mut pos, len, self.memory_ty(memory_index).index_type);
            ...
            WasmHeapTypeInner::Func => {
                ...
                let expected_type_id = {
                    ...
                    let type_ids = self.builder.ins().load(
                        ...
                        mem_flags,
                        vmctx,
                        i32::from(StaticVMContextShape.vmctx_type_ids_array()),
                    );
                    let offset =
