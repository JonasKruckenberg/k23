...
impl<I: VCodeInst> MachBuffer<I> {
    ...
    pub fn emit_island(&mut self, distance: CodeOffset) {
        self.emit_island_maybe_forced(ForceVeneers::No, distance);
    }
    ...
    fn emit_island_maybe_forced(&mut self, force_veneers: ForceVeneers, distance: CodeOffset) {
    ...
    fn finish_emission_maybe_forcing_veneers(&mut self, force_veneers: ForceVeneers) {
    ...
    pub fn finish(mut self, constants: &VCodeConstants) -> MachBufferFinalized<Stencil> {
        // let _tt = timing::vcode_emit_finish();

        self.finish_emission_maybe_forcing_veneers(ForceVeneers::No);
...
mod test {
    ...
    fn test_flip_cond() {
        ...
        let buf2 = buf2.finish(&constants);
