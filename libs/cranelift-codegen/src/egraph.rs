    ...
    'analysis: 'opt,
{
    ...
    fn optimize_pure_enode(&mut self, inst: Inst) -> Value {
        ...
        log::trace!("Incrementing rewrite depth; now {}", self.rewrite_depth);

        // Invoke the ISLE toplevel constructor, getting all new
        // values produced as equivalents to this value.
        log::trace!("Calling into ISLE with original value {}", orig_value);
        ...
        log::trace!("  -> returned from ISLE: {orig_value} -> {optimized_values:?}");
        ...
        for optimized_value in optimized_values.drain(..) {
            ...
            if optimized_value == orig_value {
                log::trace!(" -> same as orig value; skipping");
                continue;
            }
            ...
            self.stats.union += 1;
            log::trace!(" -> union: now {}", union_value);
        ...
        self.rewrite_depth -= 1;
        log::trace!("Decrementing rewrite depth; now {}", self.rewrite_depth);
    ...
    fn optimize_skeleton_inst(&mut self, inst: Inst) -> bool {
        ...
        if is_mergeable_for_egraph(self.func, inst) {
            ...
            log::trace!(" -> mergeable side-effecting op {}", inst);
                ...
                .entry((ty, self.func.dfg.insts[inst].clone()))
            {
                ScopedEntry::Occupied(o) => {
                    ...
                    self.value_to_opt_value[result] = orig_result;
                    log::trace!(" -> merges result {} to {}", result, orig_result);
                    true
                }
                ScopedEntry::Vacant(v) => {
                    ...
                    v.insert(result);
                    log::trace!(" -> inserts as new (no GVN)");
                    false
                }
            }
        }
...
impl<'a> EgraphPass<'a> {
    ...
    pub fn run(&mut self) {
        ...
        log::trace!("egraph built:\n{}\n", self.func.display());
        if cfg!(feature = "trace-log") {
            for (value, def) in self.func.dfg.values_and_defs() {
                log::trace!(" -> {} = {:?}", value, def);
                match def {
                    ValueDef::Result(i, 0) => {
                        log::trace!("  -> {} = {:?}", i, self.func.dfg.insts[i]);
                    }
                    _ => {}
                }
            }
        ...
        log::trace!("pinned_union_count: {}", self.eclasses.pinned_union_count);
        self.elaborate();
    }
        ...
        while let Some(entry) = block_stack.pop() {
            match entry {
                StackEntry::Visit(block) => {
                    ...
                    log::trace!("Processing block {}", block);
                    ...
                    for &param in cursor.func.dfg.block_params(block) {
                        log::trace!("creating initial singleton eclass for blockparam {}", param);
                        self.eclasses.add(param);
                        value_to_opt_value[param] = param;
                        available_block[param] = block;
                    }
                    while let Some(inst) = cursor.next_inst() {
                        log::trace!("Processing inst {}", inst);
                        ...
                        for &result in cursor.func.dfg.inst_results(inst) {
                            log::trace!("creating initial singleton eclass for {}", result);
                            self.eclasses.add(result);
                        }
                        ...
                        cursor.func.dfg.map_inst_values(inst, |arg| {
                            ...
                            log::trace!("rewriting arg {} of inst {} to {}", arg, inst, new_value);
