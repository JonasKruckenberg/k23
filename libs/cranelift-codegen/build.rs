// Build script.
//
// This program is run by Cargo when building cranelift-cranelift-codegen. It is used to generate Rust code from
// the language definitions in the cranelift-cranelift-codegen/meta directory.
//
// Environment:
//
// OUT_DIR
...
fn get_isle_compilations(
    ...
    // `cranelift/cranelift-codegen/src/isa/*/lower/isle/generated_code.rs`!
    Ok(IsleCompilations {
        items: vec![
    ...
    for compilation in &isle_compilations.items {
        ...
        if let Err(e) = run_compilation(compilation) {
            ...
            #[cfg(not(feature = "isle-errors"))]
            {
                ...
                eprintln!();
                eprintln!(
                    "    $ cargo check -p cranelift-cranelift-codegen --features isle-errors"
                );
                eprintln!();
            }
        }
...
///
/// NB: This must happen *after* the `cranelift-cranelift-codegen-meta` functions, since
