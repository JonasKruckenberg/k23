...
impl<const MAX_ORDER: usize> Area<MAX_ORDER> {
    ...
    pub unsafe fn deallocate(&mut self, mut block: NonNull<Frame>, layout: Layout) {
        ...
        for order in initial_order..MAX_ORDER {
            let buddy = block.map_addr(|addr| NonZeroUsize::new(addr.get() ^ (1 << order)).unwrap());

            unsafe { println!("trying to merge block {:?} with buddy {:?} at order {order}", block.as_ref(), buddy.as_ref()); }
