...
use kmem::{MemoryAttributes, VirtualAddress};
...
impl AddressSpaceRegion {
    pub const fn new(
        ...
        Self {
            range: base..base.checked_add(layout.size()).unwrap(),
            attributes,

            max_gap: 0,
            subtree_range: base..base.checked_add(layout.size()).unwrap(),
    ...
    pub fn subtree_range(&self) -> &Range<VirtualAddress> {
        &self.subtree_range
    }

    #[cfg(debug_assertions)]
    pub const fn layout_fits_region(layout: Layout, region: &Self) -> bool {
        region.range.start.is_aligned_to(layout.align())
            && layout.size() >= region.layout.size()
            && layout.size() <= region.range.end.get() - region.range.start.get()
    }

    pub fn commit(&mut self, _batch: &mut Batch, _range: impl RangeBounds<VirtualAddress>) {
        // self.vmo.

        todo!()
    }

    pub fn decommit(&mut self, _batch: &mut Batch, _range: impl RangeBounds<VirtualAddress>) {
        todo!()
    }

    pub fn set_attributes(&mut self, _batch: &mut Batch, _attributes: MemoryAttributes) {
        todo!()
    }
