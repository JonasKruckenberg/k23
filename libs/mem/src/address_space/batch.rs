...
use crate::address_space::raw::{Flush, RawAddressSpace};
        ...
        for op in self.ops.drain(..) {
            match op {
                BatchOperation::Map(op) => {
                    debug_assert!(op.virt.is_aligned_to(A::PAGE_SIZE.get()));
                    debug_assert!(op.phys.is_aligned_to(A::PAGE_SIZE.get()));
                    debug_assert!(op.len.get().is_multiple_of(A::PAGE_SIZE.get()));
...
mod test {
    #[test]
    fn maps_contiguous_same_rules() {}

    #[test]
    fn maps_contiguous_different_rules() {}

    #[test]
    fn maps_discontiguous_same_rules() {}

    #[test]
    fn maps_discontiguous_different_rules() {}
}
