...
impl<A: Arch, F: FrameAllocator<A>> FallibleIterator for FrameIterZeroed<'_, F, A> {
    ...
    fn next(&mut self) -> Result<Option<Self::Item>, Self::Error> {
        ...
        // Safety: we just allocated the frame
        unsafe {
            ptr::write_bytes::<u8>(A::phys_to_virt(base).as_mut_ptr().cast::<u8>(), 0, len.get());
        }
...
mod tests {
    ...
    fn allocate_single_page_returns_aligned_address() {
        ...
        let (addr, size) = iter.next().unwrap().unwrap();

        assert!(addr.is_aligned_to(layout.align()));
        assert_eq!(size.get(), layout.size());
    ...
    fn allocate_multiple_pages() {
        ...
        let mut count = 0;
        for (block, size) in alloc.allocate(layout).unwrap() {
            assert!(block.is_aligned_to(layout.align()));
            assert_eq!(size.get(), Arch::PAGE_SIZE);
    ...
    fn allocate_multiple_pages_multi_page_chunks() {
        ...
        for (block, size) in alloc.allocate(layout).unwrap() {
            ...
            assert!(size.get() >= Arch::PAGE_SIZE);
            // must never be larger than the alloc chunk size we set above
            assert!(size.get() <= 2 * Arch::PAGE_SIZE);
    ...
    fn allocate_zeroed_single_page_contains_zeros() {
        ...
        let memory =
            unsafe { slice::from_raw_parts(Arch::phys_to_virt(addr).as_ptr().cast::<u8>(), size.get()) };
        assert!(memory.iter().all(|&byte| byte == 0));
    }
    ...
    fn allocate_zeroed_multiple_pages_all_contain_zeros() {
        ...
        for (addr, size) in alloc.allocate_zeroed(layout).unwrap() {
            let memory =
                unsafe { slice::from_raw_parts(Arch::phys_to_virt(addr).as_ptr().cast::<u8>(), size.get()) };
            assert!(memory.iter().all(|&byte| byte == 0));
        }
    ...
    fn allocate_respects_alignment() {
        ...
        let mut iter = allocator.allocate(layout);
        let (addr, _) = iter.next().unwrap().unwrap();
