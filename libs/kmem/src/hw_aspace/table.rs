...
impl<A: Arch> Table<marker::Owned, A> {
    pub fn allocate(frame_allocator: &dyn FrameAllocator<A>) -> Result<Self, AllocError> {
    ...
    pub unsafe fn deallocate(self, frame_allocator: &dyn FrameAllocator<A>) {
        let layout = unsafe { Layout::from_size_align_unchecked(A::PAGE_SIZE, A::PAGE_SIZE) };
...
impl<'a, A: Arch> Cursor<marker::Mut<'a>, A> {
    ...
    pub fn remove_current(&mut self) -> A::PageTableEntry {
        mem::replace(self.current_entry_mut(), A::PageTableEntry::new_empty())
    }
