...
# ===== buck2-change-detector (btd) =====
#
# Files that, if changed, are too coarse for BTD to reason about and force a
# full-suite run instead. Most of these either drive third-party buckification
# (Cargo.toml/Cargo.lock + reindeer) or change toolchains/Buck cells globally.
_btd_pessimistic_paths := "Cargo.toml Cargo.lock third-party/Cargo.toml third-party/Cargo.lock flake.nix flake.lock rust-toolchain.toml .buckconfig PACKAGE"

# emit the jj summary between BASE and the working copy (raw, for BTD --changes)
btd-changes BASE:
    jj diff --summary --from {{BASE}} --to @

# print impacted Rust targets, or `__ALL__` if a pessimistic file changed
btd-impacted CHANGES BASE_JSONL UNIVERSE='root//...':
    #!/usr/bin/env bash
    set -euo pipefail
    for p in {{_btd_pessimistic_paths}}; do
